...
import uk.gov.hmrc.cipemailfrontend.views.html.ResendPasscodePage

class ResendPasscodePageViewSpec extends AnyWordSpec
  with Matchers
  ...
  private val resendPasscodePageView: ResendPasscodePage = inject[ResendPasscodePage]

  private def view: Html = resendPasscodePageView(Email.form)

  private val doc: Document = Jsoup.parse(view.body)

  override def fakeApplication(): Application = GuiceApplicationBuilder().build()

  "Resend passcode page" should {
    "display page title" in {
      doc.title() shouldBe "Request a new passcode"
    }
  }
}
